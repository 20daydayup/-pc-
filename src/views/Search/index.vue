...
export default {
  ...
  data() {
    return {
      options: {
        ...
        categoryName: "", //搜索名称
        keyword: "", //搜索关键字
        order: "1:decs", //排序方式 1：综合排序  2：价格排序   asc 升序  desc 降序
        pageNo: 1, //分页的页码
        pageSize: 10, //分页的每页商品数量
        props: [], //商品属性
        trademark: "", //品牌
      },
      isAllDown: true, //综合排序的图标
  ...
  methods: {
    ...
    updateProductList(pageNo = 1) {
      ...
      const options = {
        //一上来发送 请求会发送数据
        ...this.options, //携带初始化数据
        keyword, //下面的属性会覆盖上面的数据
        categoryName,
        category1Id,
        category2Id,
        category3Id,
    ...
    setOrder(order) {
      ...
      // 综合点两下才发生排序改变
      if (orderNum === order) {
        // 看order是1改综合排序  // 看order是2改价格排序
        if (order === "1") {
          this.isAllDown = !this.isAllDown; //取反
        } else {
          this.isPriceDown = !this.isPriceDown;
        }
        //看上一次的值，取反
        orderType = orderType === "desc" ? "asc" : "desc"; //取反
      } else {
